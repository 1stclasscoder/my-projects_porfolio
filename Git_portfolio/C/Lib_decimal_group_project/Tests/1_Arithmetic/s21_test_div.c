...
START_TEST(test_s21_div_21) {
  bit32_t max = 0b11111111111111111111111111111111;
  s21_decimal decimal_1 = {{max - 1, max, max, 0}};
  s21_decimal decimal_2 = {{0b10, 0, 0, 0}};
  decimal_1 = s21_decimal_set_scale(decimal_1, 28);
  s21_decimal result = {{0, 0, 0, 0}};
  s21_decimal check = {{max, max, 0b01111111111111111111111111111111, 0}};
  check = s21_decimal_set_scale(check, 28);
  int return_value = s21_div(decimal_1, decimal_2, &result);
  ck_assert_int_eq(s21_is_equal(result, check), 1);
  ck_assert_int_eq(return_value, S21_SUCCES);
}
...
Suite *test_s21_div_suite() {
  ...
  tcase_add_test(tc_core, test_s21_div_21);
